	...
	if (B->getID() == 3) {
		...
		if (velY > 0 && getBot() > B->getTop() - 5 && getBot() < (B->getBot() + B->getTop()) / 2) {
			if (!isSpecDown) {
				...
				velY = 0;
				posY = B->getTop() - height;
				Projectile *pj0 = new Projectile(posX + width/2, posY + 25, 6, 12, 3, 2,2);
				Projectile *pj1 = new Projectile(posX + width/2 - 10, posY + 25, 6, 12, -3, 1,2);
...
void Firebat::Atk(){
	...
	if(faceDir == 1){
		sword = new Melee(posX + width, posY + 40, 32, 45, 3, faceDir);
	}
	else{
		sword = new Melee(posX, posY + 25, 32, 45, 3, faceDir);
	}
...
void Firebat::drawTo(SDL_Surface *surf) {
	...
	//if on ground
		...
		if (isSpecial || isAtk) src.x += width * 6;
	}
	else {
